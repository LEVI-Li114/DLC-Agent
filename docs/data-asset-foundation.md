...
| 有任务映射的表 | 14 | 1506 |
| 有运行实例关联的表 | 9 | 约 1125 |
| SQLite 运行实例 | 54076 | 13996（仅最近 7 天） |
| 数据源任务中可落表的任务 | - | 913/1080 |
| 数据源到表关系 | - | 1612 |
| 质量规则 | 62（含陈旧缓存） | 60（与 WeData `TotalCount` 一致） |

按层级的任务映射覆盖为：ODS 83%、DWD 77%、DIM 76%、ADS 65%、DWS 52%。`crm_fxiaoke_tx` 的 38 个关联任务全部通过真实 `GetTask` 配置解析，得到 34 张表，未解析任务和缺字段表均为 0。

同步健康状态仍为 `partial`：全库包含大量 unknown/临时表，当前血缘覆盖 37.4%、质量规则覆盖 0.8%、任务映射覆盖 37.6%、最近 7 天实例关联覆盖 28.1%，均低于现行健康阈值。这些数字现在反映真实覆盖缺口，不再因事实表非空而误报 `ok`。
